  ...
  >
    ...
    >
      ...
      <div v-if="account.type === 'apikey'" class="space-y-4">
        ...
        <!-- Model Restriction Section (不适用于 Gemini、Antigravity 和 OpenAI Compatible) -->
        <div
          v-if="
            account.platform !== 'gemini' &&
            account.platform !== 'antigravity' &&
      ...
      <!-- Model Restriction Section for OAuth/SetupToken accounts (Anthropic only) -->
      <div
        v-if="
        ...
        <div>
          <div class="mb-3 rounded-lg bg-cyan-50 p-3 dark:bg-cyan-900/20">
            <p class="text-xs text-cyan-700 dark:text-cyan-400">
              {{ t(`admin.accounts.${account.platform}.modelMappingHint`) }}
            </p>
          </div>
...
const baseUrlHint = computed(() => {
  ...
  if (
    ...
    return t(`admin.accounts.${props.account.platform}.baseUrlHint`);
...
watch(
  ...
  (newAccount) => {
    if (newAccount) {
      ...
      // Load OpenAI Compatible model mappings
      if (
        newAccount.platform === "openai_compat" ||
        newAccount.platform === "openrouter"
      ) {
        const rawOrMapping = credentials?.model_mapping as
          | Record<string, string>
          | undefined;
        if (rawOrMapping && typeof rawOrMapping === "object") {
          openrouterModelMappings.value = Object.entries(rawOrMapping).map(
            ([from, to]) => ({ from, to })
          );
        } else {
      ...
      if (newAccount.type === "apikey" && newAccount.credentials) {
        ...
        // Load model restriction for OAuth/setup-token accounts
        const oauthCredentials = (newAccount.credentials || {}) as Record<
          string,
          unknown
        >;
        const existingMappings = oauthCredentials.model_mapping as
          | Record<string, string>
          | undefined;
        if (existingMappings && typeof existingMappings === "object") {
          const entries = Object.entries(existingMappings);
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.type === "apikey") {
      ...
      // For oauth/setup-token types, update model_mapping and intercept_warmup_requests
      const currentCredentials =
        (props.account.credentials as Record<string, unknown>) || {};
      const newCredentials: Record<string, unknown> = { ...currentCredentials };

      // Handle model restriction for oauth/setup-token accounts
      if (
    ...
    // OpenAI Compatible: persist model mapping to credentials
    if (
      props.account.platform === "openai_compat" ||
      props.account.platform === "openrouter"
    ) {
      ...
      const orModelMapping = buildModelMappingObject(
        "mapping",
        [],
        openrouterModelMappings.value
      );
      if (orModelMapping) {
        newCredentials.model_mapping = orModelMapping;
      }
...
const handleMixedChannelConfirm = async () => {
  ...
  if (pendingUpdatePayload.value && props.account) {
    ...
    submitting.value = true;
    try {
      await adminAPI.accounts.update(
        props.account.id,
