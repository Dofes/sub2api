<template>
  <div v-if="showUsageWindows">
    ...
    <!-- GLM accounts: show 5h usage from API -->
    <template v-else-if="account.platform === 'glm'">
      ...
      <div
        v-else-if="usageInfo?.five_hour || usageInfo?.seven_day"
        class="space-y-1"
      >
        <UsageProgressBar
          v-if="usageInfo?.five_hour"
...
const showUsageWindows = computed(() => {
  ...
  // GLM: always fetch usage from API (apikey type)
  if (props.account.platform === "glm") return true;
